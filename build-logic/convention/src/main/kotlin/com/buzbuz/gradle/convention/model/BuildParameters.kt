...
abstract class BuildParameters {
    ...
    /** Enables external Release signing only when explicitly requested. */
    val releaseSigningEnabled: BuildParameter<Boolean> by lazy {
        BuildParameter(rootProject = rootProject.get(), name = "releaseSigningEnabled", defaultValue = false)
    }

    /** Absolute path to an external Release signing keystore. */
    val signingStoreFile: BuildParameter<String> by lazy {
        BuildParameter(rootProject = rootProject.get(), name = "signingStoreFile", defaultValue = "")
    }
