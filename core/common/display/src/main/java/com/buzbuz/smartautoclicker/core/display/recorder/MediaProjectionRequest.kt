...
import com.buzbuz.smartautoclicker.core.capture.R
...
class MediaProjectionRequest {
     ...
     fun showMediaProjectionWarning(
          ...
          if (shouldShowDisclosure(context)) {
               AlertDialog.Builder(context)
                    .setTitle(R.string.dialog_title_media_projection_disclosure)
                    .setMessage(R.string.message_media_projection_disclosure)
                    .setPositiveButton(R.string.button_media_projection_continue) { _, _ ->
                         context.getSharedPreferences(DISCLOSURE_PREFERENCES, Context.MODE_PRIVATE)
                              .edit()
                              .putBoolean(KEY_MEDIA_PROJECTION_DISCLOSURE_ACCEPTED, true)
                              .apply()
                         requestMediaProjection(context, forceEntireScreen, onSuccess, onFailure, onError)
                    }
                    .setNegativeButton(R.string.button_media_projection_cancel) { _, _ -> onFailure() }
     ...
     private fun shouldShowDisclosure(context: Context): Boolean =
          context is Activity && !context.isFinishing && !context.isDestroyed &&
               !context.getSharedPreferences(DISCLOSURE_PREFERENCES, Context.MODE_PRIVATE)
                    .getBoolean(KEY_MEDIA_PROJECTION_DISCLOSURE_ACCEPTED, false)
     ...
     private fun requestMediaProjection(
          ...
          val projectionManager = ContextCompat
               .getSystemService(context, MediaProjectionManager::class.java) ?: run {
               onError()
               return
          }
...
private const val DISCLOSURE_PREFERENCES = "media_projection_disclosure"
private const val KEY_MEDIA_PROJECTION_DISCLOSURE_ACCEPTED = "accepted"